...
import { LoadConfig, SaveConfig, ResetShortcuts } from "../../wailsjs/go/services/CLIService";
import { GetRemotes, SaveRemote, DeleteRemote } from "../../wailsjs/go/remote/RemoteHandler";
...
export const initRemotes = async () => {
    try {
        ...
        const uiData: RemoteSource[] = data.map((d) => ({
            ...
            mappingRules: d.mappingRules ? d.mappingRules.map(m => ({
                id: m.id,
                label: m.label,
                key: m.key
            })) : [],
...
export const updateShortcutInStore = async (id: string, modifiers: string[], key: string) => {
...
export const characterSettings = writable<Record<string, CharacterSessionState>>({});
...
export function getSettingsFor(id: string, metadata: any): CharacterSessionState {
